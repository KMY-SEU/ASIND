...
def generate_A(n_nodes=100):
    ...
    # # # read edge list
    # edgelist = pd.read_csv('social_network.txt', header=None)
    #
    # # create network G
    # G = nx.Graph()
...
def generate_11NetDyns(settings):
    ...
    x[0] = np.random.random(size=settings['n_nodes']) * 0.1

    gamma = np.random.random(size=settings['n_nodes']) * 0.2
    omega = np.random.random(size=settings['n_nodes']) * 0.3

    for step in range(0, settings['n_steps'] - 1):
        x_F, x_G = generate_lib_FG(x[step], settings['func_F'], settings['func_G'])

        x_dot = x_F * omega + np.einsum('ij, ij -> i', x_G, A) * gamma
...
if __name__ == '__main__':
    ...
    print(x)
    x.to_csv('obs_SIS_barabasi_16.csv', header=False, index=False)

    #
    A = pd.DataFrame(A)
    print(A)
    A.to_csv('A_SIS_barabasi_16.csv', header=False, index=False)
