...
class SINDyBase(object):
    ...
    @staticmethod
    def generate_lib(data, degree=1):
        """
        :param data: a 2D numpy array of original features stored in each column
        :param degree: degree of polynomials of features to be expanded
        :param remove_zero_order: boolean value, indicate whether to remove the zero order term
        ...
        def x_p(x, _p):
            ...
            for i in range(n_nodes):
                _ = x[:, i] ** _p
                desc_x_p.append('x_' + str(i) + '^' + str(_p))
                lib_x_p.append(_.reshape([n_steps, 1]))
                for sub_p in range(_p - 1, 0, -1):
                    for j in range(i + 1, n_nodes):
                        _ = x[:, i] ** sub_p * x[:, j] ** (_p - sub_p)
                        lib_x_p.append(_.reshape([n_steps, 1]))
                        desc_x_p.append('x_' + str(i) + '^' + str(sub_p) + 'x_' + str(j) + '^' + str(_p - sub_p))

            return lib_x_p, desc_x_p

        lib = np.ones(shape=[n_steps], dtype=data.dtype).reshape([n_steps, 1])
        desc = ['1']
        for p in range(1, degree + 1):
            lib_x_p, desc_x_p = x_p(data, p)
            desc += desc_x_p
            for _ in lib_x_p:
                lib = np.concatenate([lib, _], axis=1)

        return np.array(lib), desc
